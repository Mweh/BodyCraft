...
struct SummaryStepView: View {
    ...
    @AppStorage("hasAcceptedTnC") private var hasAcceptedTnC: Bool = false
    @State private var showingTnC: Bool = false
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            HStack(alignment: .top, spacing: 12) {
                Button(action: { hasAcceptedTnC.toggle() }) {
                    Image(systemName: hasAcceptedTnC ? "checkmark.square.fill" : "square")
                        .foregroundColor(hasAcceptedTnC ? AppTheme.primary : AppTheme.secondaryText)
                        .font(.title3)
                }
                .buttonStyle(.plain)
                .offset(y: 2) // slightly align with first line
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("I understand the program generated by AI is not medical advice and I take full responsibility for my health.")
                        .foregroundColor(AppTheme.secondaryText)
                        .font(.caption)
            ...
            Button(action: finishAction) {
                ...
                .frame(maxWidth: .infinity)
                .padding()
                .background(hasAcceptedTnC ? AppTheme.primary : AppTheme.surface)
                .foregroundColor(hasAcceptedTnC ? .white : AppTheme.secondaryText)
                .cornerRadius(12)
            }
            .disabled(!hasAcceptedTnC)
        }
        .padding()
        .sheet(isPresented: $showingTnC) {
